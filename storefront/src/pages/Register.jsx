  ...
  return (
    <Box sx={{ minHeight: '60vh', display: 'flex', alignItems: 'center', justifyContent: 'center', p: 3 }}>
      <Paper sx={{ p: 4, maxWidth: 420, width: '100%', borderRadius: 3 }}>
        <Typography variant="h5" fontWeight={700} sx={{ mb: 3, textAlign: 'center' }}>إنشاء حساب</Typography>
        {error && <Typography color="error" sx={{ mb: 2 }}>{error}</Typography>}
        <form onSubmit={handleSubmit}>
          <TextField fullWidth label="الاسم" value={name} onChange={(e) => setName(e.target.value)} required sx={{ mb: 2 }} />
          ...
          <Button type="submit" fullWidth variant="contained" disabled={loading} sx={{ py: 1.5 }}>{loading ? 'جاري...' : 'تسجيل'}</Button>
        </form>
        <Typography sx={{ mt: 2, textAlign: 'center' }}>
          عندك حساب؟ <Link to="/login" style={{ color: 'inherit' }}>دخول</Link>
        </Typography>
      </Paper>
    </Box>
  );
