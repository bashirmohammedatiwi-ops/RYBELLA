...
export function CartProvider({ children }) {
  ...
  const addItem = async (variantId, quantity = 1, extra = {}) => {
    if (!user) {
      const stored = JSON.parse(localStorage.getItem(CART_KEY) || '[]')
      const exists = stored.find((i) => i.variant_id === variantId)
      const newItem = { variant_id: variantId, quantity, ...extra }
      const next = exists
  ...
  const updateItem = async (itemId, quantity) => {
    if (!user) {
      const stored = JSON.parse(localStorage.getItem(CART_KEY) || '[]')
      const next = stored.map((i) => (String(i.variant_id) === String(itemId) ? { ...i, quantity } : i)).filter((i) => (i.quantity || 0) > 0)
      saveGuestCart(next)
      return
    }
  ...
  const removeItem = async (itemId) => {
    if (!user) {
      const stored = JSON.parse(localStorage.getItem(CART_KEY) || '[]')
      saveGuestCart(stored.filter((i) => String(i.variant_id) !== String(itemId)))
      return
    }
  ...
  return (
    <AuthContext.Provider value={{ items, loading, addItem, updateItem, removeItem, loadCart, mergeGuestCart, totalCount }}>
      {children}
