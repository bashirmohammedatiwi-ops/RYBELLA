  ...
  return (
    <div className="premium-order-detail">
      ...
      <div className="premium-order-detail-totals">
        <div><span>التوصيل</span><span>{formatPrice(order.delivery_fee || 0)}</span></div>
        {order.discount > 0 && <div><span>الخصم</span><span>-{formatPrice(order.discount)}</span></div>}
        <div className="premium-order-final"><span>المجموع</span><strong>{formatPrice(order.final_price || 0)}</strong></div>
      </div>
    </div>
  )
}
